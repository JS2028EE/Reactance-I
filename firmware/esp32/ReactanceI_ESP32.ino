...
const char* NOTIFY_URL=""; // Set to your HTTPS /notify worker URL later.
...
void sendIR(uint8_t command){digitalWrite(PIN_IR_EMITTER,LOW);delayMicroseconds(9000);digitalWrite(PIN_IR_EMITTER,HIGH);delayMicroseconds(4500);for(int i=0;i<8;i++){digitalWrite(PIN_IR_EMITTER,LOW);delayMicroseconds(560);digitalWrite(PIN_IR_EMITTER,HIGH);delayMicroseconds((command&(1<<i))?1690:560);}digitalWrite(PIN_IR_EMITTER,HIGH);}
...
void notifyEvent(const String& msg){if(strlen(NOTIFY_URL)==0)return;HTTPClient http;if(!http.begin(NOTIFY_URL))return;http.addHeader("Content-Type","application/json");String body="{\"message\":\"";for(size_t i=0;i<msg.length();i++){char c=msg[i];if(c=='\\'||c=='\"')body+='\\';body+=c;}body+="\"}";int code=http.POST(body);http.end();if(code>=200&&code<300)logEvent(EVT_NOTIFY,code);}
